// TIM-3897: Shared inline renderer for AnalyseResponse — extracted to a neutral
// location so all Phase 5 workspaces can import from here instead of
// src/components/location-lease/. Pattern from TIM-3879.
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-white px-4 py-4 flex flex-col gap-4">
      ...
      {result.recommendations.length > 0 && (
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--foreground)] mb-2 flex items-center gap-1.5">
            <Lightbulb size={11} aria-hidden="true" />
            Recommendations
          </p>
          <ul className="flex flex-col gap-2">
            {result.recommendations.map((r, i) => (
              <li key={i} className="text-sm text-[var(--foreground)] leading-relaxed flex gap-2 items-start">
