...
export async function GET(
  ...
  const rl = await enforceRateLimit({ bucket: "copilot:thread:read", id: user.id, limit: 10, windowSec: 60 })
  if (rl) return rl
...
export async function PATCH(
  ...
  const rl = await enforceRateLimit({ bucket: "copilot:thread:write", id: user.id, limit: 10, windowSec: 60 })
  if (rl) return rl
