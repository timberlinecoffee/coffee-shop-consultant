// TIM-1690: Bulk section-assignment + position update for supplies items.
// Called by supplies-workspace onApplySuggestions after Scout's reorganize_supplies_list tool is accepted.
...
export async function POST(request: NextRequest) {
  ...
  if (failed.length > 0) {
    console.error("reorganize-supplies: some items failed to update", failed.map((u) => u.error))
