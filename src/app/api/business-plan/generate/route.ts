...
export async function POST(request: NextRequest) {
  ...
  const { data: profile } = await supabase
    .from("users")
    .select("subscription_status, subscription_tier, trial_credits_granted, ai_credits_remaining, onboarding_data, beta_waiver_until")
    .eq("id", user.id)
    .maybeSingle();
  ...
  // `creditBalance` is the post-grant balance the gate and debit both read.
  // `spendsCredits` is true for trial and paid alike (beta-waived spend nothing).
