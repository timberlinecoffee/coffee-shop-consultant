...
const MODULE_1_SECTIONS = [
  ...
  { key: "concept_brief", title: "Your Shop Summary", requiredFields: ["brief_content"] },
];
            ...
            <button
              ...
              className={`w-full text-left p-4 rounded-xl border transition-all ${
                model === m.id
                  ...
                  : "border-grey-light bg-white hover:border-neutral-500"
              }`}
            >
        ...
        {competitors.map((comp, i) => (
          <div key={i} className="bg-white border border-grey-light rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-neutral-500 mb-1">Feel (busy? quiet? hip?)</label>
                <input
                  type="text"
                  value={comp.vibe}
...
function SectionConceptBrief({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="font-semibold text-neutral-950 mb-3">Your Shop Summary</h3>
        <p className="text-sm text-neutral-500 leading-relaxed">
        ...
        {isEditing ? (
          ...
          <>
            ...
            <button
              ...
              className="mt-3 px-4 py-2 border border-teal text-teal rounded-xl text-sm font-medium hover:bg-teal/5 transition-colors"
            >
              Edit this brief
            </button>
          </>
      ...
      <div className="bg-teal/5 border border-teal/20 rounded-xl p-4">
        <p className="text-sm text-teal font-medium mb-1">PDF export coming in a future update</p>
        <p className="text-xs text-teal/70">Your brief is saved automatically. You can return to it anytime from your dashboard.</p>
      </div>
    </div>
  );
}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-grey-light">
          ...
          <div className="flex items-center gap-3">
            {subscriptionTier === "pro" ? (
              <span className="text-xs text-sage font-medium">Unlimited</span>
            ) : (
            ...
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--neutral-500)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {loading && (
            <div className="flex justify-start">
              <div data-testid="coach-thinking" className="bg-neutral-200 rounded-2xl rounded-bl-sm px-4 py-3">
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 bg-teal text-white rounded-xl hover:bg-teal-dark transition-colors disabled:opacity-40 flex-shrink-0"
                >
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {userProfile.subscription_tier === "pro" ? (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${coachOpen ? "bg-white/20 text-sage" : "bg-grey-light text-sage"}`}>∞</span>
              ) : credits > 0 ? (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${credits <= 10 ? (coachOpen ? "bg-amber-400/30 text-amber-200" : "bg-amber-100 text-amber-600") : (coachOpen ? "bg-white/20" : "bg-grey-light")}`}>{credits}</span>
              ) : null}
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-1 max-w-6xl mx-auto w-full px-6 py-8 gap-8">
          ...
          <div className="sticky top-24 space-y-1">
            ...
            {allComplete && (
              <div className="mt-4 p-3 bg-teal/10 rounded-xl">
                <p className="text-xs font-medium text-teal">Module complete!</p>
                <p className="text-xs text-teal/70 mt-0.5">All {SECTIONS.length} sections done.</p>
              </div>
            )}
          </div>
        </aside>
        ...
        <main className={`flex-1 min-w-0 transition-all duration-300 ${coachOpen ? "lg:mr-80 xl:mr-96" : ""}`}>
          ...
          <div className="bg-white rounded-2xl border border-grey-light p-6 sm:p-8">
            ...
            {/* Section nav */}
            <div className="flex items-center justify-between mt-6 pt-6 border-t border-grey-light">
              <button
      ...
      <div className="lg:hidden fixed bottom-[72px] right-4 z-30">
        <button
          ...
          className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl shadow-lg active:scale-95 transition-all ${
            coachOpen ? "bg-teal-dark text-white" : "bg-teal text-white"
          }`}
        >
