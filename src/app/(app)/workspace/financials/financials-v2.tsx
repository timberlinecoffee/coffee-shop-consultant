...
function AccordionSection({
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
      {/* TIM-3869: outer element is now a <div> instead of <button> so Phase 3+4
          aiAction buttons nested inside SectionHeader don't trigger the accordion
          toggle on click (nested <button> elements are invalid HTML). The toggle
          affordance is scoped to the chevron+title button only. */}
      <div className="w-full flex items-center justify-between px-5 py-4">
        ...
        >
          <ChevronDown
            ...
            aria-hidden="true"
          />
          <span className="text-sm font-semibold text-[var(--foreground)] truncate" title={title}>
            {title}
          </span>
        </button>
