...
test("resolvePrompt appends the brand style suffix", () => {
  const r = getRecipe("recipe-flat-white");
  const prompt = resolvePrompt(r);
  assert.ok(prompt.includes(r.subject.trim()), "prompt should contain the subject");
  assert.ok(prompt.includes("#155e63"), "prompt should carry the brand teal token");
  assert.ok(prompt.endsWith(STYLE_SUFFIX.trim()), "style suffix should be appended last");
});
...
test("cost estimate is in the documented gpt-image-1.5 range", () => {
  ...
  // hero: 1536x1024 high ~= $0.20; card: 1024x1024 medium ~= $0.034
  assert.ok(estimateCostUsd(hero) > 0.15 && estimateCostUsd(hero) <= 0.2, "hero cost out of range");
  assert.ok(estimateCostUsd(card) > 0.02 && estimateCostUsd(card) < 0.05, "card cost out of range");
});
