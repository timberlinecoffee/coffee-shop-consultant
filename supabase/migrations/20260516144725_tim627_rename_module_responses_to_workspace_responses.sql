...
alter table public.workspace_responses
  ...
  check (workspace_key in ('concept','location_lease','financials','menu_pricing','buildout_equipment','launch_plan'));
alter table public.workspace_responses
...
create policy "Users can manage own workspace responses" on public.workspace_responses for all
  using (auth.uid() = (select user_id from public.coffee_shop_plans where id = plan_id))
  with check (auth.uid() = (select user_id from public.coffee_shop_plans where id = plan_id));
