...
-- Extends schema to support: type (equipment/fund), models TEXT[], price ranges,
-- budget ranges (for fund items), bundled_with, sort_order.
-- Drops the menu_profile column (replaced by models[]).
-- Renames rationale → notes.

-- ── Schema changes ────────────────────────────────────────────────────────────

-- Drop old unique constraint (was on menu_profile + name_canonical)
...
-- Drop index on menu_profile (no longer needed)
DROP INDEX IF EXISTS public.standard_equipment_reference_menu_profile_idx;

-- Rename rationale → notes (more general; seed endpoint and UI use this label)
ALTER TABLE public.standard_equipment_reference
  RENAME COLUMN rationale TO notes;

-- Drop menu_profile column (replaced by models TEXT[])
ALTER TABLE public.standard_equipment_reference
...
-- New unique constraint on name_canonical alone
ALTER TABLE public.standard_equipment_reference
  ADD CONSTRAINT standard_equipment_reference_name_canonical_key
  UNIQUE (name_canonical);

-- New index on models for containment queries
...
-- Add the 7 catalog categories that weren't in the TIM-1005 list.
...
ALTER TABLE public.buildout_equipment_items
  ...
  CHECK (category IN (
    ...
    'to_go_ware',
    'miscellaneous',
    -- new categories from v1.0 catalog (TIM-1007)
    'cold_beverage',
    'cleaning_sanitation',
    'food_prep',
    'tech_back_office',
...
-- ── Seed: 115-item v1.0 catalog ───────────────────────────────────────────────
-- Columns: name_canonical, category, type, must_have, models, price_low, price_mid, price_high,
...
-- Bundled items — in table for research but excluded from seeded list
...
('Brew Scale (Batch & Pour-Over)',            'brew_platform', 'equipment', true,  ARRAY['FC','BO','MO','RC','DW'], 30, 90, 220, NULL, NULL, NULL, 'Larger platform than espresso scale. Separate purchase.', NULL, 104),
...
-- ── Category 3: Milk & Beverage Prep ─────────────────────────────────────────
('Milk Steaming Pitcher Set',                 'milk_beverage_prep', 'equipment', true,  ARRAY['FC','KI','DT','MO','RC','DW'], 60, 120, 250, NULL, NULL, NULL, '12oz × 4, 20oz × 4; buy 8 minimum for busy bar.', NULL, 200),
...
('Commercial High-Speed Blender',             'milk_beverage_prep', 'equipment', true,  ARRAY['FC','KI','DT','RC','DW'], 250, 750, 1600, NULL, NULL, NULL, 'For frappés, smoothies, matcha blends. Container bundled.', NULL, 204),
...
-- ── Category 5: Refrigeration ────────────────────────────────────────────────
('Under-Counter Bar Refrigerator',            'refrigeration', 'equipment', true,  ARRAY['FC','KI','DT','RC','DW'], 800, 2200, 5000, NULL, NULL, NULL, 'Primary milk and cold ingredient storage at bar. 27"–48".', NULL, 400),
...
-- ── Category 6: Cleaning & Sanitation ────────────────────────────────────────
...
('Waste Bin Set (Front + Back of House)',    'cleaning_sanitation', 'equipment', true,  ARRAY['FC','KI','DT','BO','MO','RC','DW'], 40, 120, 300,  NULL, NULL, NULL, 'Stainless step-cans (back), decorative/branded (front).', NULL, 509),

-- ── Category 7: Smallwares ────────────────────────────────────────────────────
...
('Tray Set (Service and Bussing)',           'smallwares', 'equipment', false, ARRAY['FC','BO','RC'], 40, 100, 250, NULL, NULL, NULL, '12–14" oval or rectangular for table delivery and bussing.', NULL, 606),
('Speed Rail / Bar Rail',                    'smallwares', 'equipment', true,  ARRAY['FC','KI','DT','RC','DW'], 30, 80, 200, NULL, NULL, NULL, 'Holds syrups, sauce bottles in reach at bar. 24"–36" stainless rails.', NULL, 607),

-- ── Category 8: Front-of-House Service ───────────────────────────────────────
...
-- ── Category 9: Ware Budget Funds ────────────────────────────────────────────
('Ceramics Fund',                             'ceramics', 'fund', true, ARRAY['FC','BO','RC','KI'], NULL, NULL, NULL, 500, 2500, 6000, 'Espresso cups, cappuccino cups, café latte cups, mugs, saucers. Budget 2× seat count + backup.', NULL, 800),
...
-- ── Category 11: Furniture & Seating ─────────────────────────────────────────
('Dining Tables Fund',                        'furniture_fixtures', 'fund', false, ARRAY['FC','BO','RC'], NULL, NULL, NULL, 500, 3000, 12000, 'Used furniture saves 60–70%. $50–$100 used, $200–$500 new per table.', NULL, 1000),
('Dining Chairs Fund',                        'furniture_fixtures', 'fund', false, ARRAY['FC','BO','RC'], NULL, NULL, NULL, 400, 2500, 10000, 'Allow 4–6 chairs per table plus extras. $20–$50 used, $80–$200 new per chair.', NULL, 1001),
...
-- ── Category 12: Décor / Branding ─────────────────────────────────────────────
('Opening Décor and Branding Fund',           'signage_decor', 'fund', true, ARRAY['FC','KI','DT','BO','MO','RC','DW'], NULL, NULL, NULL, 500, 3500, 15000, 'Wall art, plants, branded prints, window clings, merchandise holders. Exterior illuminated signage excluded.', NULL, 1100),
...
('Label Maker (Back-of-House)',              'tech_back_office', 'equipment', true,  ARRAY['FC','KI','DT','BO','MO','RC','DW'], 30, 80, 200, NULL, NULL, NULL, 'Date/content labels for storage. Brother PT-E110.', NULL, 1204),
...
-- ── Category 14: Miscellaneous Fund ──────────────────────────────────────────
...
-- ── Model-Specific: Drive-Thru (DT, DW) ──────────────────────────────────────
